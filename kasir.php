					...
					<div class="panel-body">
						<table class="table table-striped table-bordered table-hover" id="tabel-data">
							...
							<tbody>
								<?php
								$i = 1;
								$sql = "SELECT * FROM users WHERE role='kasir' ORDER BY nama ASC";
								...
								while ($data = mysqli_fetch_array($ress)) {
									...
									echo '<td class="text-center">
													  <a href="kasir_edit.php?kas=' . $data['id'] . '" class="btn btn-warning btn-xs">Edit</a>'; ?>
									<!-- <a href="kasir_hapus.php?kas=<?php echo $data['id']; ?>" onclick="return confirm('Apakah anda yakin akan menghapus <?php echo $data['nama']; ?>?');" class="btn btn-danger btn-xs">Hapus</a></td> -->
								<?php
									echo '</td>';
									echo '</tr>';
									$i++;
